...
void			handle_fdf_info(t_info *ifdf, t_keycode keycode)
{
...
int				key_press(int keycode, void *param)
{
	...
	{
		ifdf = get_fdf_info();
		draw_color_square(env, (t_pnt3d){780, 480, 0, BLACK}, 350);
	}
	if (keycode == KEY_ESC)
		clear_mlx_env(env);
	if (keycode == KEY_DEFAULT || key_is_direc(keycode) ||
		key_is_zoom(keycode) || key_is_z_incr(keycode) ||
		key_is_rot(keycode) || key_is_proj(keycode))
	{
